...
def open_ppecham_data(expname, data_type = 'rad', average_type = 'zonmean'):
    ...
    dset = xr.open_mfdataset( '%s/%s*%s.nc' % (data_path, average_type, data_type),
                             chunks = {'time': 1}, combine='by_coords',)

    # add experiment name as new dimension
    dset = dset.expand_dims(['expname'])
    dset['expname'] = [expname, ]
...
def open_ppecham_from_explist( explist, 
                               add_height = True, 
                               height_type = 'full',  data_type = 'rad', average_type = 'zonmean'): 
...
def read_wifiaus_inner( data_dir, file_type = 'echam', run_type = 'nudged', average_type = 'zonmean' ):
...
def read_wifiaus_nudged( data_dir, file_type = 'echam', average_type = 'zonmean'):
    ...
    return read_wifiaus_inner( data_dir, file_type = file_type, run_type = 'nudged', average_type = average_type)
...
#        data_sub_dir = f'{data_dir}/{subdir}'
        d = read_wifiaus_inner( subdir, file_type = file_type, run_type = 'ensemble', average_type = average_type)
...
def read_wifiaus_combination( base_data_dir = "/work/bb1262/data/echam-ham/wifi-aus/", 
                             file_type = 'echam', average_type = 'zonmean' ,
                             time_range = slice('2020-01', '2020-03') ):
    # ### Open Nudged Data 
    data_dir   = f"{base_data_dir}/wifiaus_ham_long_nu_gfas2020_injtrp+1_pcb_ao"
    
    nudged = read_wifiaus_nudged( data_dir, file_type = file_type, average_type = average_type )
    
    
    # ### Open Ensemble Data 
    data_dir   = f"{base_data_dir}/wifiaus_ham_long_gfas2020_injtrp+1_pcb_ao_ens"
    ensdat = read_wifiaus_ensemble( data_dir, file_type = file_type, average_type = average_type )
...
def wifiaus_add_vars( dset ):
    ...
    dset['net_toa_clear'].attrs = {'long_name': 'net TOA radiation (clear-sky)',
				   'units': 'W/m**2',}

    dset['net_toa'] = dset['srad0'] + dset['trad0']
    dset['net_toa'].attrs = {'long_name': 'net TOA radiation',
		             'units': 'W/m**2',}
